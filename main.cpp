...
int main(int argc, char* argv[]) {
    ...
    const char* mem = static_cast<const char*>(mmap(nullptr, file_size, PROT_READ, MAP_PRIVATE, fd, 0));
    ...
    while (offset < file_size) {
        ...
        __builtin_prefetch(mem + offset + 64);  // optional performance boost

        itch::dispatch(type, mem + offset, len);  // call correct handler
