...
inline void handleIPOQuotingPeriodUpdate(const char* buffer, size_t length, int log_fd) {
    ...
    msg.release_time = fast_ntohl(msg.release_time);
    msg.ipo_price = fast_ntohl(msg.ipo_price);
    ...
    // Write release time
    offset += snprintf(csv_row + offset, sizeof(csv_row) - offset, "%u,", msg.release_time);
