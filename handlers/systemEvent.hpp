...
inline void handleSystemEvent(const char* buffer, size_t length, int log_fd) {
    if (__builtin_expect(!buffer || length < sizeof(SystemEventMessage), 0)) {
        std::cerr << "Error: Invalid buffer size for System Event message\n";
        return;
    }

    SystemEventMessage msg = itch::PackedParser<SystemEventMessage>::parse(buffer, length);
