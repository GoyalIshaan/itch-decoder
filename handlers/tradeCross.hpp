...
inline void handleTradeCross(const char* buffer, size_t length, int log_fd) {
    ...
    msg.shares = fast_ntohll(msg.shares);
    msg.cross_price = fast_ntohl(msg.cross_price);
    ...
    // Write shares
    offset += snprintf(csv_row + offset, sizeof(csv_row) - offset, "%llu,", msg.shares);
    ...
    // Write cross price
    offset += snprintf(csv_row + offset, sizeof(csv_row) - offset, "%u,", msg.cross_price);
