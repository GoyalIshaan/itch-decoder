...
inline void handleBrokenTrade(const char* buffer, size_t length, int log_fd) {
    if (__builtin_expect(!buffer || length < sizeof(BrokenTradeMessage), 0)) {
        std::cerr << "Error: Invalid buffer size for Broken Trade message\n";
        return;
    }

    BrokenTradeMessage msg = itch::PackedParser<BrokenTradeMessage>::parse(buffer, length);
